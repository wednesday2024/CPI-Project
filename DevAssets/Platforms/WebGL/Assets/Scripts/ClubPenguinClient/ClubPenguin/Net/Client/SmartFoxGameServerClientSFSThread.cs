    ...
    public class SmartFoxGameServerClientSFSThread
    {
        ...
        public void fetchEncryptionKey()
        {
            ...
            try
            {
                ...
                {
#if UNITY_WEBGL
                    // [PATCH] If keys are missing in WebGL, log as Warning (not Error) and proceed without encryption
                    UnityEngine.Debug.LogWarning("[WebGL] RSA parameters not available. Proceeding without encryption.");
                    roomJoinCompleted();
                    return;
#else
            // In standalone, this is a genuine error
...
#if UNITY_WEBGL
                // Fallback: join room even if encryption setup crashes
                roomJoinCompleted();
#endif
            }
        }
