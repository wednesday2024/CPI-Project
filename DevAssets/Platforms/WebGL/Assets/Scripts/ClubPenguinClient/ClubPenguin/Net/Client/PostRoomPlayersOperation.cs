...
namespace ClubPenguin.Net.Client
{
	...
	public class PostRoomPlayersOperation : CPAPIHttpOperation
	{
		...
		public static SignedResponse<JoinRoomData> JoinRoom(string world, string language, ZoneId zoneId, OfflineDatabase offlineDatabase, IOfflineDefinitionLoader offlineDefinitions)
		{
			...
			playerRoomData.dailyTaskProgress = SetTaskProgressOperation.GetProgressList(offlineDatabase, offlineDefinitions);
			...
			RoomIdentifier roomIdentifier = new RoomIdentifier(world, LocalizationLanguage.GetLanguageFromLanguageString(language), zoneId, new ContentIdentifier("1.13.5", "offline", DateTime.UtcNow.ToString("yyyy-MM-dd"), "NONE").ToString());
