...
namespace IceFishing
{
	public class mg_if_GameLogic : MonoBehaviour
	{
        ...
        public void Start()
        {
            ...
            m_resultShown = false;
            FishCaught = 0;
            m_inputManager = gameObject.AddComponent<mg_if_InputManager>();
            m_inputManager.Initialize(MinigameManager.GetActive().MainCamera, this);
            m_initialized = true;
        }

        public void OnDestroy()
        {
            // Remove the call to TidyUp, since mg_if_InputManager does not have this method
            m_inputManager = null;
        }
