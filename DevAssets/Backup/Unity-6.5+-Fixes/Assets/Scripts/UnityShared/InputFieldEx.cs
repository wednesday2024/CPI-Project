...
public class InputFieldEx : InputField
{
	...
	private void Update()
	{
		...
		bool anyKeyDown = Keyboard.current != null && Keyboard.current.anyKey.wasPressedThisFrame;
		if (anyKeyDown || base.text.Length > 0)
		{
			nothingEnteredYet = false;
		}
		bool returnKey = Keyboard.current != null && (Keyboard.current.enterKey.wasPressedThisFrame || Keyboard.current.numpadEnterKey.wasPressedThisFrame);
		if (returnKey)
		{
			onKeyboardDone();
		}
