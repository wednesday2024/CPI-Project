...
public class TextWithEvents : Text
{
	...
	protected override void OnPopulateMesh(VertexHelper vh)
	{
		...
		if (charsIdForClass.Count != 0)
		{
			...
			if (textGenerator == null || textGenerator.lineCount == 0 || textGenerator.lines == null || textGenerator.lines.Count == 0 || textGenerator.verts.Count == 0)
			{
				return;
			}
			...
			foreach (KeyValuePair<string, List<int[]>> item in charsIdForClass)
			{
				...
				{
					if (item2 == null || item2.Length < 2 || textGenerator.vertexCount == 0)
					{
						continue;
					}
					
					if (textGenerator.lineCount > 1 && textGenerator.verts.Count > item2[0] * 4 + 2 && textGenerator.verts.Count > item2[1] * 4 + 3 && textGenerator.verts[item2[0] * 4 + 2].position.y > textGenerator.verts[(item2[1] * 4 + 3 > textGenerator.vertexCount - 1) ? (textGenerator.vertexCount - 4) : (item2[1] * 4)].position.y)
					{
						int num = 1;
						for (int i = 0; i < textGenerator.lineCount - 1; i++)
						{
							if (textGenerator.lines == null || i < 0 || i + 1 >= textGenerator.lines.Count)
							{
								break;
							}
