...
namespace Fabric
{
    ...
    public class AudioComponent : Component
    {
        ...
        private static AudioType GetDynamicAudioClipType(string audioClipPath)
        {
            string text = audioClipPath.ToLowerInvariant();
            if (text.EndsWith(".aif") || text.EndsWith(".aiff"))
            {
        ...
        private static bool HasUnityWebRequestError(UnityWebRequest request)
        {
        ...
        private IEnumerator LoadDynamicAudioClipFromFile(AudioClipHandle audioClipHandle)
        {
            string path = "";
            ...
            string audioClipPath = audioClipHandle.GetAudioClipPath();
            string uri = "file://" + path + "//" + audioClipPath;
            using (UnityWebRequest unityWebRequest = UnityWebRequestMultimedia.GetAudioClip(uri, GetDynamicAudioClipType(audioClipPath)))
            {
                yield return unityWebRequest.SendWebRequest();
                if (HasUnityWebRequestError(unityWebRequest))
                {
                    audioClipHandle.AudioClip = null;
                    DebugLog.Print("AudioComponent [ " + base.name + " ] failed to load audio clip [" + audioClipPath + "]: " + unityWebRequest.error, DebugLevel.Error);
                    yield break;
                }
                audioClipHandle.AudioClip = DownloadHandlerAudioClip.GetContent(unityWebRequest);
            }
        }
        ...
        private IEnumerator DynamicAudioClipLoad(AudioClipHandle audioClipHandle)
        {
            ...
            {
                yield return LoadDynamicAudioClipFromFile(audioClipHandle);
            }
