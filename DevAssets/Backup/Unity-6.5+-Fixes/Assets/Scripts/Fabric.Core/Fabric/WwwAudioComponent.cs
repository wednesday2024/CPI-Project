...
namespace Fabric
{
    ...
    public class WwwAudioComponent : AudioComponent
    {
        ...
        {
            get { return _audioClipReference; }
        ...
        private void Load()
        {
            ...
            www = UnityWebRequestMultimedia.GetAudioClip(text, _audioType);
            www.SendWebRequest();

            while (!www.isDone) { }

            base.AudioClip = DownloadHandlerAudioClip.GetContent(www);
        ...
        private IEnumerator LoadPlayCoroutine(ComponentInstance zComponentInstance, float target, float curve, bool dontPlayComponents)
        {
            ...
            www = UnityWebRequestMultimedia.GetAudioClip(filename, _audioType);
