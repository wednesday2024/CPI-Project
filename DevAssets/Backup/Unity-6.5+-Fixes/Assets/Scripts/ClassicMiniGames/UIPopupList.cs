...
public class UIPopupList : UIWidgetContainer
{
	...
	// Update OnKey to support Unity's new Input System
	private void OnKey(KeyCode key)
	{
		...
		// Support new Input System as well as legacy KeyCode
		...
		// Use either KeyCode or new Input System key
		if (key == KeyCode.UpArrow || up)
		{
			if (num > 0)
			{
		...
		else if (key == KeyCode.DownArrow || down)
		{
		...
		else if (key == KeyCode.Escape || escape)
		{
			OnSelect(false);
		}
	}
