...
namespace ClubPenguin.Task
{
    public class TaskService
    {
        ...
        {
            return knownTasks.TryGetValue(taskId, out definition);
        }

        public List<string> GetTaskNamesByGroup(TaskDefinition.TaskGroup group, ICollection<string> excludedNames, int maxCount)
        {
            List<string> matches = new List<string>();
            foreach (KeyValuePair<string, TaskDefinition> entry in knownTasks)
            {
                if (excludedNames != null && excludedNames.Contains(entry.Key))
                {
                    continue;
                }
            ...
            {
                matches.RemoveRange(maxCount, matches.Count - maxCount);
            }
            return matches;
        }
