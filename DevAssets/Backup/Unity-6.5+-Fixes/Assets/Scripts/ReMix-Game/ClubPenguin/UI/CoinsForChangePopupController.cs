...
namespace ClubPenguin.UI
{
	public class CoinsForChangePopupController : AnimatedPopup
	{
		...
		private void Start()
		{
			...
			dispatcher.DispatchEvent(new CFCPopupEvents.CFCPopupOpened());
		...
		{
			dispatcher.DispatchEvent(new CFCPopupEvents.CFCPopupClosed());
		...
		private bool onDonationPosted(ScheduledEventServiceEvents.CFCDonationPosted evt)
		{
			...
			CoinsData coinsData = dataEntityCollection.GetComponent<CoinsData>(localPlayerHandle);
			
			coinsData.RemoveCoins(lastDonationAmount);
