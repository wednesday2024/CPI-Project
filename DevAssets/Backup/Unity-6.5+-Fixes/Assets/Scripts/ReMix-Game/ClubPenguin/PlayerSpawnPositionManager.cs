    ...
    public class PlayerSpawnPositionManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (!playerDataHandle.Handle.IsNull &&
                dataEntityCollection.HasComponent<LocalPlayerInZoneData>(playerDataHandle.Handle))
            {
                spawnLocalPlayerInZone();
            }
            else
            {
                eventDispatcher.AddListener<PlayerSpawnedEvents.LocalPlayerReadyToSpawn>(
                    onLocalPlayerReadyToSpawn, EventDispatcher.Priority.FIRST);
            }

            eventDispatcher.AddListener<SceneTransitionEvents.TransitionComplete>(
        ...
        public void SpawnPlayer(SpawnPlayerParams spawnPlayerParams)
        {
            if (spawnPlayerParams.Zone != null || !string.IsNullOrEmpty(spawnPlayerParams.SceneName))
            {
                ...
                PresenceData component =
                    dataEntityCollection.GetComponent<PresenceData>(dataEntityCollection.LocalPlayerHandle);

                ZoneTransitionService zoneTransitionService = Service.Get<ZoneTransitionService>();

                if (spawnPlayerParams.Zone != null)
                {
                    ZoneDefinition zoneBySceneName =
                        zoneTransitionService.GetZoneBySceneName(component.Room);

                    flag = (zoneBySceneName != null &&
                            string.Equals(spawnPlayerParams.Zone.ZoneName, zoneBySceneName.ZoneName));
                }
                else if (!string.IsNullOrEmpty(spawnPlayerParams.SceneName))
                {
                    flag = (!zoneTransitionService.IsInIgloo)
                        ? string.Equals(component.Room, spawnPlayerParams.SceneName)
                        : string.Equals(zoneTransitionService.CurrentZone.SceneName,
                                        spawnPlayerParams.SceneName);
                }
                ...
                if (!flag)
                {
                    ...
                    {
                        component2 = dataEntityCollection.AddComponent<SpawnData>(
            ...
            if (spawnPlayerParams.PendingReward != null)
            {
                Service.Get<EventDispatcher>().DispatchEvent(
                    new RewardServiceEvents.MyRewardEarned(
                        RewardSource.QUICK_NOTIFICATION, "", spawnPlayerParams.PendingReward));
            }
            ...
            else if (spawnPlayerParams.GetOutOfSwimming &&
                     component3.IsCurrentControllerOfType<SwimController>())
            {
            ...
            if (spawnPlayerParams.NudgePlayer &&
                component3.IsCurrentControllerOfType<RunController>())
            {
                zero.y = 0.5f;
            }

            base.transform.SetPositionAndRotation(
				spawnPlayerParams.Position + zero,
            ...
            if (spawnPlayerParams.NudgePlayer)
            {
                ...
                CoroutineRunner.Start(
                    LocomotionUtils.nudgePlayer(component3, onNudgePlayerDone),
                    component3.gameObject,
                    "MoveAfterJump");
            }
        ...
        {
            eventDispatcher.RemoveListener<SceneTransitionEvents.TransitionComplete>(
                onSceneTransitionComplete);
        ...
        {
            eventDispatcher.RemoveListener<PlayerSpawnedEvents.LocalPlayerReadyToSpawn>(
                onLocalPlayerReadyToSpawn);
        ...
        private void executeSpawedAction(SpawnedAction spawnedAction)
        {
            if (spawnedAction.Reward != null)
            {
                Service.Get<EventDispatcher>().DispatchEvent(
                    new RewardServiceEvents.MyRewardEarned(
                        RewardSource.QUICK_NOTIFICATION, "", spawnedAction.Reward));
            }
            ...
            switch (spawnedAction.Action)
            {
                case SpawnedAction.SPAWNED_ACTION.StartQuest:
                    if (spawnedAction.Quest != null)
                    {
                        ...
                        Service.Get<EventDispatcher>().DispatchEvent(
                            new QuestEvents.StartQuest(spawnedAction.Quest));
                    }
                    break;
                ...
                case SpawnedAction.SPAWNED_ACTION.ReplayQuest:
                    if (spawnedAction.Quest != null)
                    {
                        Service.Get<TutorialManager>().EndTutorial();
                        Service.Get<EventDispatcher>().DispatchEvent(
                            new QuestEvents.ReplayQuest(spawnedAction.Quest));
                    }
                    break;
            }
        }
        ...
        private void spawnLocalPlayerInZone()
        {
            ...
            if (playerDataHandle.Handle == dataEntityCollection.LocalPlayerHandle)
            {
                ...
                else if (dataEntityCollection.TryGetComponent(
                            dataEntityCollection.LocalPlayerHandle, out component2))
                {
                    ...
                    dataEntityCollection.RemoveComponent<SpawnData>(
                        dataEntityCollection.LocalPlayerHandle);
                }

                if (flag &&
                    dataEntityCollection.TryGetComponent(
                        dataEntityCollection.LocalPlayerHandle, out component3))
                {
                    ...
                    {
                        eventDispatcher.AddListener<HudEvents.HudInitComplete>(
                            onHudInitCompleted);
                    }
                    return;
                }
            ...
            {
                Service.Get<EventDispatcher>().AddListener<
                    FriendsServiceEvents.FriendLocationInRoomReceived>(
                    onFriendLocationReceived);

                Service.Get<EventDispatcher>().AddListener<
                    FriendsServiceEvents.FriendNotInRoom>(
                    onFriendNotInRoom);

                Service.Get<INetworkServicesManager>().FriendsService
                    .GetFriendLocationInRoom(
                        dataEntityCollection.GetComponent<SpawnAtPlayerData>(
                            playerDataHandle.Handle).PlayerSWID);
            ...
            component3 =
                dataEntityCollection.GetComponent<PositionData>(playerDataHandle.Handle);
            ...
            if (!flag2 && component3 != null && component3.Position != Vector3.zero)
            {
                ...
                Service.Get<INetworkServicesManager>().PlayerActionService
        ...
        {
            bool flag = false;
            ZoneDefinition previousZone =
                Service.Get<ZoneTransitionService>().PreviousZone;
        ...
        private bool isSpawnNearPlayer()
        {
            ...
            return dataEntityCollection.GetComponent<SpawnAtPlayerData>(
                playerDataHandle.Handle) != null;
        }

        private bool onFriendNotInRoom(FriendsServiceEvents.FriendNotInRoom evt)
        {
            Service.Get<EventDispatcher>().RemoveListener<
                FriendsServiceEvents.FriendLocationInRoomReceived>(
                onFriendLocationReceived);
            ...
            if (!playerDataHandle.Handle.IsNull)
            {
                dataEntityCollection.RemoveComponent<SpawnAtPlayerData>(
                    playerDataHandle.Handle);
            }
        ...
        private bool spawnNearTransition(ZoneDefinition previousZone)
        {
            ...
            ZoneTransition[] array = Object.FindObjectsByType<ZoneTransition>();

            foreach (ZoneTransition zoneTransition in array)
            {
                if (zoneTransition.Zone == null)
                {
                    Log.LogErrorFormatted(this,
                        "Invalid zone transition found while attempting to spawn player. No zone definition assigned to transition '{0}'",
                        zoneTransition.GetPath());
                }
            ...
            if (gameObject != null)
            {
                SpawnPointSelector component =
                    gameObject.GetComponent<SpawnPointSelector>();
            ...
            else
            {
                MapTransition mapTransition =
                    Object.FindAnyObjectByType<MapTransition>();

                if (mapTransition != null)
                {
                    SpawnPointSelector component =
                        mapTransition.GetComponent<SpawnPointSelector>();
        ...
        private bool spawnNearDefaultLocation()
        {
            ...
            DefaultSpawnPointSelector spawnPointSelector =
                Object.FindAnyObjectByType<DefaultSpawnPointSelector>();
        ...
        private IEnumerator setPositionToSpawnPointWaitAFrame(SpawnPointSelector spawner)
        {
            ...
            LocomotionTracker tracker =
                SceneRefs.ZoneLocalPlayerManager.LocalPlayerGameObject
                    .GetComponent<LocomotionTracker>();
            ...
            LocomotionActionEvent movement = default;
        ...
        private void sendNetworkMessage(LocomotionActionEvent action)
        {
            ...
            {
                Service.Get<INetworkServicesManager>()
                    .PlayerActionService.LocomotionAction(action);
            }
        }
    }
}