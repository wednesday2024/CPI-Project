	...
	public class CellPhoneController : MonoBehaviour
	{
		...
		private bool onChangeScreen(CellPhoneEvents.ChangeCellPhoneScreen evt)
		{
			...
			case CellPhoneAppBehaviour.LoadSettings:
			{
				...
				bool useSettingsScene = PlatformUtils.GetPlatformType() == PlatformType.Mobile && (PlatformUtils.GetAspectRatioType() == AspectRatioType.Portrait || (evt.AppletSceneSystemMemoryThreshold >= 0 && num <= evt.AppletSceneSystemMemoryThreshold));
				if (useSettingsScene)
				{
