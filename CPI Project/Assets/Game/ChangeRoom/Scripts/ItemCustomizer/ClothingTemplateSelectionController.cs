	...
	public class ClothingTemplateSelectionController : TemplateSelectionController
	{
		...
		private void parseItemGroups()
		{
			...
			foreach (QuestDefinition knownQuest in questService.KnownQuests)
			{
				...
				string mascotName = value != null ? value.Name : "";
		...
		{
			if (rewardTemplates.Exists((DisplayedTemplate x) => x.Definition.Id == newDisplayTemplate.Definition.Id)) return;
		...
		private void filterDisplayedTemplatesByTags(List<DisplayedTemplate> filterTemplates)
		{
			...
			if (filterTags != null && filterTags.Count > 0)
			{
				for (int i = 0; i < filterTemplates.Count; i++)
				{
					...
					for (int tagIndex = 0; tagIndex < filterTags.Count; tagIndex++)
					{
		...
		private bool onTemplateSelected(CustomizerUIEvents.SelectTemplate evt)
		{
			if (!loadingOverlay.activeSelf)
			{
				...
				var spriteToUse = evt.TemplateSprite;
				if (spriteToUse == null)
				{
					Debug.LogWarning("Template sprite is null in onTemplateSelected; passing null to SetTemplate.");
					// You can assign a fallback sprite here if you want, e.g.:
					// spriteToUse = defaultFallbackSprite;
				}

				itemModel.SetTemplate(evt.TemplateData, spriteToUse);
			}
			return false;
		}
