...
namespace ClubPenguin.ClothingDesigner
{
    public class ClothingDesignerController
    {
        ...
        public void Init(ClothingDesignerDependencies dependencies)
        {
            ...
            penguinLighting = null;
            if (dependencies.CustomizerAvatarPreview != null)
            {
                Transform[] componentsInChildren = dependencies.CustomizerAvatarPreview.GetComponentsInChildren<Transform>(true);
                for (int i = 0; i < componentsInChildren.Length; i++)
                {
                    Transform transform2 = componentsInChildren[i];
                    if (transform2 != null && transform2.name == PENGUIN_LIGHT_SOURCE_NAME)
                    {
                        penguinLighting = transform2.gameObject;
                        break;
                    }
                }
            }
