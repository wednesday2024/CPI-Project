...
namespace ClubPenguin.World.Activities.Diving
{
    public class DivingGameController : MonoBehaviour
    {
        ...
        private ParticleSystem[] bubbleParticleEffects;
        ...
        private IEnumerator InitializeBubbleCoroutine(string path, GameObject prefab)
        {
            ...
            bubbleParticleEffects = bubble.GetComponentsInChildren<ParticleSystem>(true);
            ...
            if (isLocalPlayer)
            {
                ...
                if (DivingUnlimitedAir)
                {
                    ...
                    setBubbleParticles(true);
                    dispatcher.DispatchEvent(new DivingEvents.FreeAirEffects(true, base.gameObject.tag));
                }
                yield break;
            }
        ...
        private void setBubbleParticles(bool enabled)
        {
            if (bubbleParticleEffects == null) return;
            foreach (ParticleSystem ps in bubbleParticleEffects)
            {
                if (ps == null) continue;
                if (enabled)
                {
        ...
        private BubbleState getStartingBubbleState(float airSupply)
        {
            if (DivingUnlimitedAir)
            {
                return BubbleState.TransitionToPlentifulAir;
            }
