	...
	public class SlideController : LocomotionController, ISlideControllerAdapter
	{
		...
		public override void Steer(Vector2 steerInput)
		{
			if (mode == Mode.PhysicsDriven)
			{
				...
				steerVel *= (isFloatingOnWater ? mutableData.ImpulseScaleOnWater * 30 * Time.fixedDeltaTime : mutableData.ImpulseScale * 30 * Time.fixedDeltaTime);
		...
		{
			steerVel = wsSteerInput * (isFloatingOnWater ? mutableData.ImpulseScaleOnWater * 30 * Time.fixedDeltaTime : mutableData.ImpulseScale * 30 * Time.fixedDeltaTime);
			base.Broadcaster.BroadcastOnSteerDirectionEvent(steerVel);
		}
