...
namespace ClubPenguin.CellPhone
{
	public class ActivityNotificationManager : MonoBehaviour
	{
		...
		private DateTime nextNotificationDateTime;
		...
		private void onScheduleLoaded(string path, ActivityNotificationSchedule schedule)
		{
			...
			int currentMinuteInDay = dateTime.Minute + dateTime.Hour * 60;
			bool flag = false;
			for (int i = 0; i < orderedNotificationBlocks.Count; i++)
			{
				if (orderedNotificationBlocks[i].TriggerTime > currentMinuteInDay)
				{
					...
					nextNotificationDateTime = dateTime.Date.AddMinutes(orderedNotificationBlocks[i].TriggerTime);
					flag = true;
					break;
				}
			}
			if (!flag && orderedNotificationBlocks.Count > 0)
			{
				nextNotificationIndex = 0;
				nextNotificationDateTime = dateTime.Date.AddDays(1.0).AddMinutes(orderedNotificationBlocks[0].TriggerTime);
				flag = true;
			}
			if (flag)
			{
		...
		private void onTimerTick()
		{
			DateTime dateTime = Service.Get<ContentSchedulerService>().PresentTime();
			int currentMinuteInDay = dateTime.Minute + dateTime.Hour * 60;
			if (dateTime >= nextNotificationDateTime)
			{
