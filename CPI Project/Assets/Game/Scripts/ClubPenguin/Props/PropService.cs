...
namespace ClubPenguin.Props
{
	public class PropService
	{
		...
		public class PartySupplyTypeGenerator : NamedToggleValueAttribute.NamedToggleValueGenerator
		{
			public IEnumerable<NamedToggleValueAttribute.NamedToggleValue> GetNameToggleValues()
			{
				List<NamedToggleValueAttribute.NamedToggleValue> list = new List<NamedToggleValueAttribute.NamedToggleValue>();
		...
		[Invokable("Inventory.Consumable.RemoveInventoryCount", Description = "Remove a number of party supplies of the given type from your inventory")]
		[PublicTweak]
		private void removeConsumableTypeCount([NamedToggleValue(typeof(PartySupplyTypeGenerator), 0u)] string type, int count)
		{
			int num = Math.Max(0, count);
			setConsumableTypeCountInternal(type, getConsumableTypeCount(type) - num);
		}

		[Invokable("Inventory.Consumable.ClearPartySupplies", Description = "Remove all party supplies from your inventory")]
		[PublicTweak]
		private void clearPartySupplyInventory()
		{
			foreach (PropDefinition value in Props.Values)
			{
				if (isPartySupplyType(value))
				{
					setConsumableTypeCountInternal(value.GetNameOnServer(), 0);
				}
			}
		}

        [Invokable("Inventory.Consumable.MaxPartySupplies", Description = "Add all party supplies to your inventory")]
        [PublicTweak]
        private void maxPartySupplyInventory()
        {
            foreach (PropDefinition value in Props.Values)
            {
                if (isPartySupplyType(value))
                {
                    setConsumableTypeCountInternal(value.GetNameOnServer(), 999);
                }
            }
        }

        private static bool isPartySupplyType(PropDefinition propDefinition)
		{
			return propDefinition.PropType == PropDefinition.PropTypes.Consumable && !propDefinition.ServerAddedItem && propDefinition.ExperienceType != PropDefinition.ConsumableExperience.PartyGameLobby;
		}

		private static int getConsumableTypeCount(string type)
		{
			...
			ConsumableInventory consumableInventory = component.ConsumableInventory;
			if (consumableInventory.inventoryMap == null)
			{
				return 0;
			}
			InventoryItemStock value;
			if (!consumableInventory.inventoryMap.TryGetValue(type, out value))
			{
				return 0;
			}
		...
		private static void setConsumableTypeCountInternal(string type, int count)
		{
			Service.Get<INetworkServicesManager>().ConsumableService.QA_SetTypeCount(type, Math.Max(0, count));
		}
	}
}
