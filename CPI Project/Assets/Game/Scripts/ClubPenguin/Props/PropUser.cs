...
namespace ClubPenguin.Props
{
	...
	public class PropUser : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				locomotionStateSetter.ActionButtonInvoked += OnActionButtonInvoked;
			}
			else
			{
			}
		...
		private void OnLocomotionBroadcasterDoAction(LocomotionController.LocomotionAction action, object userData = null)
		{
			...
			case LocomotionController.LocomotionAction.Action3:
				...
				OnPropAction(action.ToString());
				if (Prop.PropDef != null && Prop.PropDef.PropType == PropDefinition.PropTypes.InteractiveObject && Prop.IsOwnerLocalPlayer)
				{
		...
		private void OnActionButtonInvoked(ClubPenguin.Net.Domain.LocomotionAction action)
		{
			if (Prop == null)
			{
				return;
			...
			switch (action)
			{
			case ClubPenguin.Net.Domain.LocomotionAction.Action1:
			case ClubPenguin.Net.Domain.LocomotionAction.Action2:
			case ClubPenguin.Net.Domain.LocomotionAction.Action3:
				locomotionTracker.SetCurrentController<RunController>();
				OnPropAction(action.ToString());
				break;
			}
