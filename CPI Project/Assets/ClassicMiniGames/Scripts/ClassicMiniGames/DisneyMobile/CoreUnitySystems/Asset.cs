...
namespace DisneyMobile.CoreUnitySystems
{
    public class Asset
    {
        ...
        public bool ProcessResult(bool success, UnityWebRequest inwww)
        {
            LoadedObject = null;
            if (success)
            {
                switch (AssetType)
                {
                    case ResourceAssetType.AUDIO:
                        LoadedObject = DownloadHandlerAudioClip.GetContent(inwww);
                        break;
                    case ResourceAssetType.ASSET_BUNDLE:
                        LoadedObject = DownloadHandlerAssetBundle.GetContent(inwww);
                        break;
                    case ResourceAssetType.BYTES:
                        LoadedObject = inwww.downloadHandler.data;
                        break;
                    case ResourceAssetType.IMAGE:
                        LoadedObject = DownloadHandlerTexture.GetContent(inwww);
                        break;
                    case ResourceAssetType.NONSTREAMING_AUDIO:
                        LoadedObject = DownloadHandlerAudioClip.GetContent(inwww);
                        break;
                    case ResourceAssetType.TEXT_ASSET:
                        LoadedObject = inwww.downloadHandler.text;
                        break;
                    case ResourceAssetType.STRING:
                        LoadedObject = inwww.downloadHandler.text;
                        break;
