...
public class UICamera : MonoBehaviour
{
    ...
    {
        get { return (currentCamera != null && currentTouch != null) ? currentCamera.ScreenPointToRay(currentTouch.pos) : default(Ray); }
    }
    ...
    public static GameObject selectedObject
    {
        ...
        set { SetSelection(value, currentScheme); }
    }
    ...
    {
        if (Keyboard.current != null && Keyboard.current[ToInputSystemKey(up)].wasPressedThisFrame)
            return 1;
        if (Keyboard.current != null && Keyboard.current[ToInputSystemKey(down)].wasPressedThisFrame)
            return -1;
        return 0;
    }
    ...
    {
        if ((Keyboard.current != null && Keyboard.current[ToInputSystemKey(up0)].wasPressedThisFrame)
            || (Keyboard.current != null && Keyboard.current[ToInputSystemKey(up1)].wasPressedThisFrame))
            return 1;
        if ((Keyboard.current != null && Keyboard.current[ToInputSystemKey(down0)].wasPressedThisFrame)
            || (Keyboard.current != null && Keyboard.current[ToInputSystemKey(down1)].wasPressedThisFrame))
            return -1;
        return 0;
    }
    ...
    private static int GetDirection(string axis)
    {
        ...
        {
            // For Mouse scroll, try Mouse.current.scroll
            float axis2 = 0f;
            if (axis == "Mouse ScrollWheel" && Mouse.current != null)
                axis2 = Mouse.current.scroll.ReadValue().y;
            // For other axes, user must map InputActions
            if (axis2 > 0.75f)
            {
    ...
    private void Awake()
    {
        ...
        mMouse[0].pos.x = Mouse.current != null ? Mouse.current.position.ReadValue().x : 0;
        mMouse[0].pos.y = Mouse.current != null ? Mouse.current.position.ReadValue().y : 0;
        for (int i = 1; i < 3; i++)
        {
    ...
    private void Update()
    {
        ...
        {
            if (cancelKey0 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(cancelKey0)].wasPressedThisFrame)
            {
                currentScheme = ControlScheme.Controller;
                currentKey = cancelKey0;
                selectedObject = null;
            }
            else if (cancelKey1 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(cancelKey1)].wasPressedThisFrame)
            {
                currentScheme = ControlScheme.Controller;
        ...
        {
            float num = (Mouse.current != null) ? Mouse.current.scroll.ReadValue().y : 0f;
            if (num != 0f)
                Notify(mHover, "OnScroll", num);
            if (showTooltips && mTooltipTime != 0f && (mTooltipTime < RealTime.time || (Keyboard.current != null && (Keyboard.current[Key.LeftShift].isPressed || Keyboard.current[Key.RightShift].isPressed))))
            {
                mTooltip = mHover;
                ShowTooltip(true);
            }
    ...
    {
        lastTouchPosition = Mouse.current != null ? Mouse.current.position.ReadValue() : Vector2.zero;
        ...
        for (int i = 0; i < 3; i++)
        {
            bool down = Mouse.current != null && ((i == 0 && Mouse.current.leftButton.wasPressedThisFrame) || (i == 1 && Mouse.current.rightButton.wasPressedThisFrame) || (i == 2 && Mouse.current.middleButton.wasPressedThisFrame));
            bool held = Mouse.current != null && ((i == 0 && Mouse.current.leftButton.isPressed) || (i == 1 && Mouse.current.rightButton.isPressed) || (i == 2 && Mouse.current.middleButton.isPressed));
            if (down)
            {
        ...
        for (int i = 0; i < 3; i++)
        {
            bool mouseButtonDown = Mouse.current != null && ((i == 0 && Mouse.current.leftButton.wasPressedThisFrame) || (i == 1 && Mouse.current.rightButton.wasPressedThisFrame) || (i == 2 && Mouse.current.middleButton.wasPressedThisFrame));
            bool mouseButtonUp = Mouse.current != null && ((i == 0 && Mouse.current.leftButton.wasReleasedThisFrame) || (i == 1 && Mouse.current.rightButton.wasReleasedThisFrame) || (i == 2 && Mouse.current.middleButton.wasReleasedThisFrame));
    ...
    public void ProcessTouches()
    {
        ...
        for (int i = 0; i < touchCount; i++)
        {
            ...
            int fingerId = i; // Touchscreen touches are indexed, but you may want to use touch.touchId.ReadValue() for actual finger id
            currentTouchID = ((!allowMultiTouch) ? 1 : fingerId);
            currentTouch = GetTouch(currentTouchID);
            UnityEngine.InputSystem.TouchPhase phase = touch.phase.ReadValue();
            bool flag = phase == UnityEngine.InputSystem.TouchPhase.Began || currentTouch.touchBegan;
            bool flag2 = phase == UnityEngine.InputSystem.TouchPhase.Canceled || phase == UnityEngine.InputSystem.TouchPhase.Ended;
            currentTouch.touchBegan = false;
            Vector2 position = touch.position.ReadValue();
            currentTouch.delta = (flag ? Vector2.zero : (position - currentTouch.pos));
            ...
            if (flag)
                ...
                currentCamera = currentTouch.pressedCam;
            // tapCount is not available directly; implement double-tap logic if needed.
    ...
    private void ProcessFakeTouches()
    {
        ...
        bool mouseButtonUp = Mouse.current != null && Mouse.current.leftButton.wasReleasedThisFrame;
        bool mouseButton = Mouse.current != null && Mouse.current.leftButton.isPressed;
    ...
    public void ProcessOthers()
    {
        ...
        bool flag = false;
        bool flag2 = false;
        if (submitKey0 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(submitKey0)].wasPressedThisFrame)
        {
            currentKey = submitKey0;
            flag = true;
        }
        if (submitKey1 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(submitKey1)].wasPressedThisFrame)
        {
            currentKey = submitKey1;
            flag = true;
        }
        if (submitKey0 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(submitKey0)].wasReleasedThisFrame)
        {
            currentKey = submitKey0;
            flag2 = true;
        }
        if (submitKey1 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(submitKey1)].wasReleasedThisFrame)
        {
            currentKey = submitKey1;
            flag2 = true;
        }
        ...
        if (useKeyboard && Keyboard.current != null && Keyboard.current[Key.Tab].wasPressedThisFrame)
        {
        ...
        if (cancelKey1 != 0 && Keyboard.current != null && Keyboard.current[ToInputSystemKey(cancelKey1)].wasPressedThisFrame)
        {
            currentKey = cancelKey1;
            currentScheme = ControlScheme.Controller;
            Notify(mCurrentSelection, "OnKey", KeyCode.Escape);
    ...
    // Utility to convert KeyCode to UnityEngine.InputSystem.Key
    private static Key ToInputSystemKey(KeyCode keyCode)
    {
        switch (keyCode)
        {
            ...
            case KeyCode.JoystickButton0: return Key.Space; // Map to Space or properly map as needed
            case KeyCode.JoystickButton1: return Key.Enter; // Map to Enter or properly map as needed
