	...
	public class mg_jr_TurboBar : MonoBehaviour
	{
		...
		private void Update()
		{
			if (!MinigameManager.IsPaused)
			{
				...
				// Use Input System for input detection
				bool firePressed = false, jumpPressed = false;
				if (Keyboard.current != null)
				{
					firePressed = Keyboard.current.spaceKey.wasPressedThisFrame || Keyboard.current.enterKey.wasPressedThisFrame;
					jumpPressed = Keyboard.current.spaceKey.wasPressedThisFrame;
				}
				if (Gamepad.current != null)
				{
					firePressed |= Gamepad.current.buttonSouth.wasPressedThisFrame; // A
					jumpPressed |= Gamepad.current.buttonSouth.wasPressedThisFrame; // A
				}
