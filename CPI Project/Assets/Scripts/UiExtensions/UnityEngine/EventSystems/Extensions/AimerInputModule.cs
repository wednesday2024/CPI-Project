...
namespace UnityEngine.EventSystems.Extensions
{
    ...
    public class AimerInputModule : PointerInputModule
    {
        ...
        // Add this to allow linking a custom input action in the inspector
        public InputActionReference activateActionReference;
        ...
        {
            bool buttonDown = IsActivateButtonDown();
            bool buttonUp = IsActivateButtonUp();
        ...
        private bool IsActivateButtonDown()
        {
            if (activateActionReference != null && activateActionReference.action != null)
            {
                return activateActionReference.action.WasPressedThisFrame();
            }

            // Fallback to default "Submit" if activateAxis is "Submit"
            if (activateAxis == "Submit")
            {
                var keyboard = Keyboard.current;
                var gamepad = Gamepad.current;
                return (keyboard != null && (keyboard[Key.Enter].wasPressedThisFrame || keyboard[Key.Space].wasPressedThisFrame)) ||
                       (gamepad != null && gamepad.buttonSouth.wasPressedThisFrame);
            }

            // If you want to support other named actions, you could add more handling here
            return false;
        }

        private bool IsActivateButtonUp()
        {
            if (activateActionReference != null && activateActionReference.action != null)
            {
                return activateActionReference.action.WasReleasedThisFrame();
            }
            ...
            if (activateAxis == "Submit")
            {
                var keyboard = Keyboard.current;
                var gamepad = Gamepad.current;
                return (keyboard != null && (keyboard[Key.Enter].wasReleasedThisFrame || keyboard[Key.Space].wasReleasedThisFrame)) ||
