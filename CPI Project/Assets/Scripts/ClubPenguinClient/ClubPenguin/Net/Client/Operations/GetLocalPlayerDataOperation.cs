...
namespace ClubPenguin.Net.Client.Operations
{
	...
	public class GetLocalPlayerDataOperation : CPAPIHttpOperation
	{
		...
		protected override void PerformOfflineAction(OfflineDatabase offlineDatabase, IOfflineDefinitionLoader offlineDefinitions)
		{
			...
			long storedDay = 0L;
			string storedDayStr = PlayerPrefs.GetString(GetPlatformKey("OfflineMinigameProgress_Day"), "0");
			long.TryParse(storedDayStr, out storedDay);
			if (storedDay != num)
			{
				PlayerPrefs.SetString(GetPlatformKey("OfflineMinigameProgress_Day"), num.ToString());
				PlayerPrefs.DeleteKey(GetPlatformKey("OfflineMinigameProgress_fishing"));
				PlayerPrefs.Save();
			}
			int fishingCount = PlayerPrefs.GetInt(GetPlatformKey("OfflineMinigameProgress_fishing"), 0);
