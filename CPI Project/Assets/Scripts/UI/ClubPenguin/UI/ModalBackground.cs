...
namespace ClubPenguin.UI
{
    ...
    public class ModalBackground : MonoBehaviour
    {
        ...
        private void pushModal()
        {
            ...
            // Deactivate previous modal background if present
            if (modalBackgrounds.Count > 0)
            {
                GameObject prev = modalBackgrounds.Peek();
                if (prev != null)
        ...
        private void popModal()
        {
            ...
            // Only remove if this modalBackground is on top of the stack
            if (modalBackgrounds.Count > 0 && modalBackgrounds.Peek() == modalBackground)
            {
                modalBackground.SetActive(false);
                modalBackgrounds.Pop();
            }
            // Remove any nulls left in the stack
            while (modalBackgrounds.Count > 0 && modalBackgrounds.Peek() == null)
            {
                modalBackgrounds.Pop();
            }
            if (modalBackgrounds.Count > 0)
            {
                GameObject top = modalBackgrounds.Peek();
                if (top != null)
                    top.SetActive(true);
            }
        }
        ...
        private void OnDestroy()
        {
            if (modalBackground != null)
            {
                // Remove from stack if this instance is still in it
                if (modalBackgrounds.Count > 0 && modalBackgrounds.Peek() == modalBackground)
                {
                    modalBackgrounds.Pop();
                    // Activate the next modal if present
                    while (modalBackgrounds.Count > 0 && modalBackgrounds.Peek() == null)
                        modalBackgrounds.Pop();
                    if (modalBackgrounds.Count > 0)
                    {
