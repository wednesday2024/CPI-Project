...
namespace LitJson
{
    public class JsonMapper
    {
        ...
        private static bool TryGetGenericDictionaryTypes(Type t, out Type keyType, out Type valueType)
        {
            keyType = null;
            valueType = null;
        ...
        private static object ConvertDictionaryKey(string keyString, Type keyType)
        {
            ...
            {
                long v;
                if (long.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v))
                    return v;
                return Convert.ToInt64(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(int))
            {
                int v2;
                if (int.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v2))
                    return v2;
                return Convert.ToInt32(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(uint))
            {
                uint v3;
                if (uint.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v3))
                    return v3;
                return Convert.ToUInt32(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(ulong))
            {
                ulong v4;
                if (ulong.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v4))
                    return v4;
                return Convert.ToUInt64(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(short))
            {
                short v5;
                if (short.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v5))
                    return v5;
                return Convert.ToInt16(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(ushort))
            {
                ushort v6;
                if (ushort.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v6))
                    return v6;
                return Convert.ToUInt16(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(byte))
            {
                byte v7;
                if (byte.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v7))
                    return v7;
                return Convert.ToByte(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }

            if (keyType == typeof(sbyte))
            {
                sbyte v8;
                if (sbyte.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out v8))
                    return v8;
                return Convert.ToSByte(Convert.ToDouble(keyString, CultureInfo.InvariantCulture));
            }
            ...
            if (keyType.IsEnum)
            {
                try
                {
                    return Enum.Parse(keyType, keyString, true);
                }
                catch
                {
                    long enumBacking;
                    if (long.TryParse(keyString, NumberStyles.Integer, CultureInfo.InvariantCulture, out enumBacking))
                        return Enum.ToObject(keyType, enumBacking);
                    throw;
                }
            }
        ...
        private static object ReadValue(Type inst_type, JsonReader reader)
        {
            ...
            else if (reader.Token == JsonToken.ObjectStart)
            {
                ...
                Type dictKeyType;
                Type dictValueType;
                bool isGenericDict = TryGetGenericDictionaryTypes(type, out dictKeyType, out dictValueType);
                ...
                while (true)
                {
                    ...
                    {
                        object keyObj;
                        if (isGenericDict && dictKeyType != null)
                        {
                            keyObj = ConvertDictionaryKey(text, dictKeyType);
                        }
                        else
                        {
                            keyObj = text;
                        ...
                        Type valueT = isGenericDict && dictValueType != null ? dictValueType : objectMetadata.ElementType;
                        ((IDictionary)obj).Add(keyObj, ReadValue(valueT, reader));
                    }
                }
            }
            return obj;
