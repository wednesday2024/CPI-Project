...
namespace ClubPenguin.UI
{
	public class ExchangeScreenInventory : MonoBehaviour
	{
		...
		{
			if (items == null || items.Count == 0 || tubes == null || tubes.Length == 0)
				return;

			List<ExchangeScreenInventoryItem> validItems = new List<ExchangeScreenInventoryItem>();
			int rowStartIndex = currentAnimatingRow * numCols;
			int rowEndIndex = Mathf.Min(rowStartIndex + numCols, items.Count);

			for (int i = rowStartIndex; i < rowEndIndex; i++)
			{
				if (items[i].ExchangeItem.CanExchange())
				{
			...
			int selectedItemIndex = UnityEngine.Random.Range(0, validItems.Count);
			GameObject item = validItems[selectedItemIndex].gameObject;

			int num = tubes.Length;
			int num2 = Mathf.FloorToInt((float)(numCols - num) * 0.5f);
			int num3 = selectedItemIndex;
			...
			if (num5 <= num4)
			{
				num5 = Mathf.Min(num4 + 1, num);
			}

			int tubeIndex = UnityEngine.Random.Range(num4, num5);
			tubeIndex = Mathf.Clamp(tubeIndex, 0, tubes.Length - 1);

			playItemAnimation(item, tubes[tubeIndex]);
		}
