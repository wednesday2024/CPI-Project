	...
	public static class RewardPopupScreenBuilder
	{
		...
		private static List<DRewardPopupScreen> buildQuestsScreen(DRewardPopup popupData, bool checkForNonMemberScreens)
		{
			...
			if (popupData.PopupType == DRewardPopup.RewardPopupType.levelUp)
			{
				...
				Mascot mascot = null;
				int num = 0;
				if (mascotService != null && !string.IsNullOrEmpty(popupData.MascotName))
				{
					mascot = mascotService.GetMascot(popupData.MascotName);
					if (mascot != null)
					{
						num = Service.Get<ProgressionService>().MascotLevel(mascot.Name);
					}
				}
				if (mascotService != null && popupData.MascotName == "AuntArctic" && num == 1)
				{
				...
				if (list.Count < 2 && mascot != null && mascot.KnownQuests != null)
				{
