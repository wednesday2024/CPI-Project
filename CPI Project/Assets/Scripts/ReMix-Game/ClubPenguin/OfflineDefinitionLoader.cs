...
namespace ClubPenguin
{
    public class OfflineDefinitionLoader : IOfflineDefinitionLoader
    {
        ...
        public Reward GetInRoomReward(List<string> newRewards)
        {
            ...
            {
                GameObject gameObject = findGameObjectByPath(newReward);
                if (gameObject != null)
                {
                    Collectible[] array = gameObject.GetComponents<Collectible>();
                    if (array.Length == 0)
                    {
                        array = gameObject.GetComponentsInParent<Collectible>(true);
                    }
                    if (array.Length == 0)
                    {
                        ...
                        SceneryCollectible[] array2 = UnityEngine.Object.FindObjectsByType<SceneryCollectible>(FindObjectsSortMode.None);
        ...
        private static GameObject findGameObjectByPath(string path)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (transform != null && transform.gameObject.scene.IsValid() && transform.gameObject.scene.isLoaded && transform.gameObject.GetPath() == path)
                {
        ...
        public int GetEquipmentTemplateDefinitionCost(int templateDefinitionId)
        {
            Dictionary<int, TemplateDefinition> dictionary = Service.Get<GameData>().Get<Dictionary<int, TemplateDefinition>>();
            TemplateDefinition value;
            if (dictionary.TryGetValue(templateDefinitionId, out value))
        ...
        {
            ActivityNotificationManager activityNotificationManager = UnityEngine.Object.FindFirstObjectByType<ActivityNotificationManager>();
