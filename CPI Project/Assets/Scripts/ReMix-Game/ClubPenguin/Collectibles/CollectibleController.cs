	...
	public class CollectibleController : MonoBehaviour
	{
		...
		private RespawnResponse getRespawnState(Collectible scriptObj)
		{
			...
			if (list[0].Collectible.SpawnCategory == SpawnCategory.Daily)
			{
				...
				long todayMidnightMs = GetTime.CurrentUtcMidnightInMilliseconds();
				if (collectedTime < todayMidnightMs)
				{
					return new RespawnResponse(RespawnState.READY_FOR_PICKUP, 0L);
				}
				return new RespawnResponse(RespawnState.WAITING_TO_RESPAWN, GetTime.NextUtcMidnightInMilliseconds());
			}
