...
namespace ClubPenguin.Diving
{
    ...
    public class Chomper : ProximityBroadcaster
    {
        ...
        {
            get { return isPenguinGrabbed; }
            set { isPenguinGrabbed = value; }
        }
        ...
        private void Update()
        {
            ...
            float y = Mathf.MoveTowardsAngle(
                base.gameObject.transform.eulerAngles.y,
                newDirection.y,
        ...
        {
            return (state == ChomperState.LUNGE)
                ? new Vector3(penguinObj.transform.position.x, penguinObj.transform.position.y, base.transform.position.z)
                : new Vector3(penguinObj.transform.position.x, base.transform.position.y, base.transform.position.z);
        }
        ...
        public void OnLungeComplete()
        {
            if (chomperState == ChomperState.LUNGE)
            {
                chomperState = IsPenguinGrabbed ? ChomperState.LUNGE_SUCCESS : ChomperState.LUNGE_MISS;
            }
        }
